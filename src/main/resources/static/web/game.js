...
function getShipLocation() {
  for (var i = 0; i < info.ship.length; i++) {
    for (var k = 0; k < info.ship[i].locations.length; k++) {
      ...
      putShipImage(
        document.getElementById("gameTable").rows[indexRow(shipLocation)].cells[
          indexCell(shipLocation)
        ],
        info.ship[i].shipType
      );
    }
  }
}
  ...
  for (var i = 0; i < info.gamePlayers.length; i++) {
    if (info.id == info.gamePlayers[i].id) {
      ...
      document.getElementById("playerName1").innerHTML =
        info.gamePlayers[i].player.firstName +
        " " +
        info.gamePlayers[i].player.lastName;
      document.getElementById("gameinformation1").innerHTML =
        email + " " + "(you)";
    } else {
      email = info.gamePlayers[i].player.email;
      ...
      if (currentPlayerId == info.salvoes[i].player) {
        info.history.forEach((history) => {
          if (history.hitLocation.includes(salvoLocation)) {
            document.getElementById("gameTableSalvo").rows[
              indexRow(salvoLocation)
            ].cells[indexCell(salvoLocation)].innerHTML =
              '<img src="giphy.gif"height=30px width=30px ></img>';
          } else {
            document.getElementById("gameTableSalvo").rows[
              indexRow(salvoLocation)
        ...
        info.ship.forEach((s) => {
          if (!isHit) {
            if (s.locations.includes(info.salvoes[i].locations[k])) {
              document.getElementById("gameTable").rows[
                indexRow(salvoLocation)
              ].cells[indexCell(salvoLocation)].innerHTML =
                '<img src="giphy.gif"height=30px width=30px ></img>';
              document.getElementById("gameTable").rows[
                indexRow(salvoLocation)
              ].cells[indexCell(salvoLocation)].style.backgroundColor =
                "#78C0EF";
              isHit = true;
            } else {
              document.getElementById("gameTable").rows[
...
function fillCell() {
  ...
  for (let [shipType, shipLength] of maps.entries()) {
    document.getElementById("gameTableShip").rows[i].cells[1].innerHTML =
      shipType;
    document.getElementById("gameTableShip").rows[i].cells[2].innerHTML =
      shipLength;
    i = i + 1;
  }
}
...
function clickedCells(clickedCell, i) {
  ...
  if (numberOfSelectedShip > 0) {
    rowId = clickedCell.parentElement.getElementsByTagName("th")[0].innerHTML;
    cellId = clickedCell.cellIndex;
  }
  //control green(if there is a green cell dont paint)
  for (var k = 0; k < selectedShipLength; k++) {
    if (
      table.rows[rowId].cells[cellId + k].style.backgroundColor == "#78C0EF"
    ) {
      return;
    }
  }
  ...
  if (
    selectedShipOnShipTable != null &&
    numberOfSelectedShip > 0 &&
    parseInt(cellId) + parseInt(selectedShipLength) <= 11
  ) {
    var shiplocation = [];
    for (var k = 0; k < selectedShipLength; k++) {
...
function returnShipButton() {
  ...
  if (horizontal == true) {
    ...
    for (var k = 1; k < selectedShipLength; k++) {
      if (
        table.rows[parseInt(rowId) + k].cells[cellId].style.backgroundColor ==
        "#78C0EF"
      ) {
    ...
    for (var k = 0; k < selectedShipLength; k++) {
      //table.rows[rowId].cells[cellId + k].style.backgroundColor = 'green';
      putShipImage(table.rows[rowId].cells[cellId + k], shipType);
      table.rows[parseInt(rowId) + k + 1].cells[cellId].style.backgroundColor =
        "transparent";
...
function clickedCellsSalvo(clickedCell, i) {
  if (salvo < 5) {
    var rowId =
      clickedCell.parentElement.getElementsByTagName("th")[0].innerHTML;
...
function paintHitShip() {
  for (let [type, hit] of lookup.entries()) {
    ...
    if (type == "Aircraft Carrier") {
      for (var i = 1; i <= hit; i++) {
        document.getElementById("hitShipTable").rows[0].cells[
          i
        ].style.backgroundColor = "#DC7E4C";
      }
    }

    if (type == "Battleship") {
      for (var i = 1; i <= hit; i++) {
        document.getElementById("hitShipTable").rows[1].cells[
          i
        ].style.backgroundColor = "#DC7E4C";
      }
    }
    if (type == "Submarine") {
      for (var i = 1; i <= hit; i++) {
        document.getElementById("hitShipTable").rows[2].cells[
          i
        ].style.backgroundColor = "#DC7E4C";
      }
    }
    if (type == "Destroyer") {
      for (var i = 1; i <= hit; i++) {
        document.getElementById("hitShipTable").rows[3].cells[
          i
        ].style.backgroundColor = "#DC7E4C";
      }
    }
    if (type == "Patrol Boat") {
      for (var i = 1; i <= hit; i++) {
        document.getElementById("hitShipTable").rows[4].cells[
          i
        ].style.backgroundColor = "#DC7E4C";
      }
    }
...
function putShipImage(cell, shipType) {
  if (shipType == "Aircraft Carrier") {
    cell.innerHTML =
      '<img src="Aircraft carrier.png" height=20px width=25px></img>';
